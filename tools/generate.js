...
fs.createReadStream('./tools/input.csv')
    ...
    .on('data', function(row) {
        ...
        var validation = [];
        ['id', 'name', 'link', 'logo', 'gn', '90 day return', 'nrf', 'no restocking fee', 'sl', 'shipping label', 'frs', 'free return shipping', 'last update', 'category'].forEach(v => {
            switch(row[v]) {
                case '':
                case NaN:
        ...
        if (row['bg'] !== "" && !(row['bg'].startsWith("#") || row['bg'].length !== 7)) {
    ...
    .on('end', function() {        
        ...
        if (validation.length === 0) {
            fs.writeFileSync('./src/assets/data.json', JSON.stringify(data), { encoding: 'utf-8', flag: 'w'});
